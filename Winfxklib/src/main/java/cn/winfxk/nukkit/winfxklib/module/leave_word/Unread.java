...
import cn.winfxk.nukkit.winfxklib.module.leave_word.reading.Reading;
import cn.winfxk.nukkit.winfxklib.tool.Tool;

import java.util.HashMap;
import java.util.Map;

public class Unread extends LeaveWordForm {
    ...
    public Unread(Player player, BaseFormin Update, boolean isBack) {
    ...
    public boolean MakeForm() {
        ...
        for (Map.Entry<String, Object> entry : Unread.entrySet()) {
            if (!(entry.getValue() instanceof Map)) continue;
            Map<String, Object> item = (Map<String, Object>) entry.getValue();
            form.addButton(message.getSun(MainKey, FormKey, "ItemKey", ItemKey, new Object[]{player.getName(), myPlayer.getMoney(), item.get("Title"), item.get("Date"), item.get("Player"), item.get("Content"), getStreamline(Tool.objToString(item.get("Content")))}), (a, b) -> show(new Reading(a, this, true, entry.getKey())));
        }
